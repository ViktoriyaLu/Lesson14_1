...
{
function topFilmsList(fields) {
    let isActiveDatas = [];
    for (let i = 0; i < datas.length; i++) {
        let data = {};
        for (let j = 0; j < fields.length; j++)
            data[fields[j]] = datas[i][fields[j]];
        isActiveDatas.push(data);
    }
    return isActiveDatas;
}
console.log(topFilmsList(["topRatedFilm"]));

const topFilmList = document.createElement('p');
topFilmList.classList.add('topRatedFilm');
topFilmList.textContent = element.topRatedFilm;
    filmsEl.appendChild(topFilmList);
}
